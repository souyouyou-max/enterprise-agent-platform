...
import com.sinosig.aip.business.chat.toolkit.AgentOrchestrationToolkit;
import com.sinosig.aip.business.chat.toolkit.ConversationOcrToolkit;
...
public class InteractionCenterAgent extends BaseAgent {
    ...
    private final AipChatProperties chatProperties;
    private final ChatToolDecisionService toolDecisionService;
    ...
    public InteractionCenterAgent(LlmService llmService,
                                  ...
                                  AipChatProperties chatProperties,
                                  ChatToolDecisionService toolDecisionService) {
    ...
    public InteractionResult chat(String sessionId, String userMessage) {
        ...
        String mode = useOrgTools ? "ORG_TOOLS" : "PLAIN";
        try {
            ...
            log.info("[InteractionCenter] chat 完成, sessionId={}, mode={}, length={}, costMs={}",
                    sessionId, mode, response.length(), (System.nanoTime() - startNs) / 1_000_000);
        } catch (Exception e) {
            ...
            log.info("[InteractionCenter] chat 降级完成, sessionId={}, mode={}, length={}, costMs={}",
                    sessionId, mode, response == null ? 0 : response.length(), (System.nanoTime() - startNs) / 1_000_000);
        }
    ...
    public Flux<String> chatStream(String sessionId, String userMessage) {
        ...
        final String mode = useOrgTools ? "ORG_TOOLS" : "PLAIN";
        ...
        return finalStreamSpec.stream()
                ...
                .switchIfEmpty(Flux.defer(() -> {
                    ...
                    log.warn("[InteractionCenter] chatStream 空响应，降级为非流式调用后分片返回, sessionId={}, mode={}",
                            sessionId, mode);
                ...
                .onErrorResume(e -> {
                    log.error("[InteractionCenter] chatStream 最终兜底失败, sessionId={}, mode={}: {}",
                            sessionId, mode, e.getMessage(), e);
                    return Flux.just("抱歉，模型服务暂时不可用，请稍后重试。");
                })
                .doOnNext(chunk -> log.debug("[InteractionCenter] stream chunk: {} chars", chunk.length()))
                .doOnComplete(() -> log.info("[InteractionCenter] chatStream 完成, sessionId={}, mode={}, costMs={}",
                        sessionId, mode, (System.nanoTime() - startNs) / 1_000_000))
     ...
     * <p>
     * 关键词列表由 {@code aip.pipeline.chat.org-tool-keywords} 配置，支持 Nacos 动态刷新。
     */
    private boolean shouldUseTools(String message) {
        return toolDecisionService.shouldUseOrgTools(message, chatProperties);
    }
