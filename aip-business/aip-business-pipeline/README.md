...
### 配置（`com.sinosig.aip.business.pipeline.config`）

| 类 | 说明 |
|----|------|
| `AipPipelineProperties` | 流水线全局配置（前缀 `aip.pipeline`）：OCR 开关、分析开关、对比开关、调度参数、提示词模板 |
...
### REST API（`com.sinosig.aip.business.pipeline.controller`）
...
### 服务层（`com.sinosig.aip.business.pipeline.service`）
...
### 领域服务（`com.sinosig.aip.business.pipeline.service.impl`）
...
### 调度器（`com.sinosig.aip.business.pipeline.scheduler`）
...
- 依赖：`aip-common`、`aip-data-repository`、`aip-engine-tools`（ZhengyanPlatformTool、DazhiOcrTool）、`aip-engine-llm`、`aip-engine-rule`（BidAnalysisClient）
- 被依赖：`aip-app`（运行时装配）、`aip-app/PipelineOpsPlaybookService`（读取配置类）
