...
import com.sinosig.aip.business.pipeline.service.DocumentImageConverter;
...
import com.sinosig.aip.tools.impl.DazhiOcrTool;
...
public class MultimodalServiceImpl implements MultimodalService {
    ...
    @Value("${aip.tools.zhengyan.platform.img2text.max-images-per-call:2}")
    private int maxImagesPerCall;

    @Value("${aip.business.chat.auto-ocr.invoke-mode:direct}")
    ...
    public ObjectNode img2Text(ObjectNode request) throws Exception {
        ...
        boolean disableBatch = request.path("disableBatch").asBoolean(false);
        JsonNode toolNode;
        if (!disableBatch && attachments.isArray() && !attachments.isEmpty()) {
    ...
    private JsonNode callImg2TextInBatches(ObjectNode request) throws Exception {
        ...
        ArrayNode batchContentsArr  = objectMapper.createArrayNode();
        ArrayNode mergedResponses   = objectMapper.createArrayNode();
        ArrayNode batchImageBase64s = objectMapper.createArrayNode();
        // ── 逐批成功标志，与 response / batchImageBase64s 数组下标严格对齐
        //    供 OcrRecognitionServiceImpl.persistZhengyanFromResult 按批独立判断，
        //    避免"任意批次失败 → 全部 split 报批次0失败"的误判。
        ArrayNode batchSuccessArr   = objectMapper.createArrayNode();
        ...
        for (int i = 0; i < imageDataUrls.size(); i += step) {
            ...
            // 每个 batch 对应一条 split；保存该 batch 使用的图片 base64（JSON 数组字符串形态入库）
            ...
            boolean thisBatchOk = node.path("success").asBoolean(false);
            batchSuccessArr.add(thisBatchOk);           // 记录本批独立结果
            if (!thisBatchOk) {
                // 把 API 层返回的原始错误信息打到日志，便于排查（配置缺失、鉴权失败、超时等）
                String batchErrMsg = node.path("message").asText(
                        node.path("result").asText("(无错误详情，请检查配置或 API 响应)"));
                log.warn("[MultimodalService] img2text 批次 {} 失败: {}", i / step, batchErrMsg);
            }

            String content = extractImg2TextContent(node);
            batchContentsArr.add(content);
            if (!content.isBlank()) {
                contents.add("[第" + startPageNo + "-" + endPageNo + "页]\n" + content);
            }
            allSuccess   = allSuccess && thisBatchOk;
        ...
        // 每批大模型正文（无页眉前缀），供流水线按分片回写 ocr_file_split
        merged.set("batchContents",      batchContentsArr);
        // 逐批成功标志随结果一起传给持久化层，下标与 response 数组对齐
        merged.set("batchSuccesses",   batchSuccessArr);
        return merged;
    }
    ...
    private ObjectNode buildImg2TextView(JsonNode toolNode) {
        ...
        // 逐批成功标志：由 callImg2TextInBatches 写入，必须透传给持久化层，
        // 使 persistZhengyanFromResult 能按批次独立判断是否入库，而非依赖全局 success。
        view.set("batchSuccesses", toolNode.path("batchSuccesses"));
        JsonNode bc = toolNode.path("batchContents");
        if (bc.isArray() && bc.size() > 0) {
            view.set("batchContents", bc);
        } else {
            ArrayNode single = objectMapper.createArrayNode();
            single.add(extractImg2TextContent(toolNode));
            view.set("batchContents", single);
        }
        return view;
    }
