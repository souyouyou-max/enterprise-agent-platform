...
import com.sinosig.aip.business.pipeline.controller.dto.SubmitBatchRequest;
import com.sinosig.aip.business.pipeline.controller.dto.SubmitBatchResponse;
import com.sinosig.aip.business.pipeline.service.OcrPipelineService;
import com.sinosig.aip.business.pipeline.service.OcrPipelineService.BatchProgressView;
 ...
 * <h3>接口清单</h3>
 * <pre>
 * POST /api/v1/aip/pipeline/submit-batch      提交多文件批次（启动完整流水线）
 * GET  /api/v1/aip/pipeline/batch/{batchNo}   查询批次处理进度
 * POST /api/v1/aip/pipeline/batch/{batchNo}/trigger-ocr       手动触发 OCR 阶段
 * POST /api/v1/aip/pipeline/batch/{batchNo}/trigger-analysis  手动触发分析阶段
 * POST /api/v1/aip/pipeline/batch/{batchNo}/trigger-compare   手动触发对比阶段
 * POST /api/v1/aip/pipeline/file/{mainId}/retry-ocr           单文件重试 OCR
 * POST /api/v1/aip/pipeline/file/{mainId}/retry-analysis      单文件重试分析
 * </pre>
 */
@Slf4j
@Tag(name = "OCR 流水线", description = "多文件 OCR → 多模态语义分析 → 文件相似度对比 全流程管理")
@RestController
@RequestMapping("/api/v1/aip/pipeline")
...
public class OcrPipelineController {
    ...
    public ResponseResult<SubmitBatchResponse> submitBatch(@RequestBody SubmitBatchRequest req) {
        ...
        String batchNo = req.resolveBatchNo();
        var batch = pipelineService.submitBatch(batchNo, req.getAppCode(), req.toFileInfoList(), req.getExtraInfo());
        log.info("[PipelineController] 批次提交成功 batchNo={}, files={}", batchNo, req.getFiles().size());
        return ResponseResult.success(SubmitBatchResponse.from(batch));
    }
