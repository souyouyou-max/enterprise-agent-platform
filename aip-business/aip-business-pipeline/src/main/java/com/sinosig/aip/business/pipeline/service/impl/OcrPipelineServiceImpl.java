...
import com.sinosig.aip.business.pipeline.config.PipelineEffectiveConfig;
import com.sinosig.aip.business.pipeline.config.PipelineEffectiveConfigResolver;
import com.sinosig.aip.business.pipeline.config.PipelineMultimodalPromptResolver;
import com.sinosig.aip.business.pipeline.event.OcrPipelineEvent;
...
import com.sinosig.aip.business.pipeline.service.OcrPipelineService;
...
import com.sinosig.aip.data.mapper.OcrFileAnalysisMapper;
import com.sinosig.aip.data.mapper.OcrPipelineBatchMapper;
...
public class OcrPipelineServiceImpl implements OcrPipelineService {
    ...
    public OcrPipelineBatch submitBatch(String batchNo, String appCode,
        ...
        // 发布批次提交事件（结构化日志）
        publishEvent(OcrPipelineEvent.builder()
                .taskId(batchNo)
                .appCode(appCode)
                .previousStatus(null)
                .currentStatus(OcrPipelineBatch.STATUS_PENDING)
                .eventType("SUBMITTED")
                .totalFiles(files.size())
                .message("批次已提交，共 " + files.size() + " 个文件")
                .build());
    ...
    public void triggerOcrPhase(String batchNo) {
        ...
        // 顺序固定：OCR → 分析 → 对比；全局 aip.pipeline 与 extra_info 合并后的开关决定跳过
    ...
    private void markBatchDone(Long batchId) {
        OcrPipelineBatch batch = batchMapper.selectById(batchId);
        String prevStatus = batch != null ? batch.getStatus() : null;
        ...
        publishEvent(OcrPipelineEvent.builder()
                .taskId(batch != null ? batch.getBatchNo() : String.valueOf(batchId))
                .appCode(batch != null ? batch.getAppCode() : null)
                .previousStatus(prevStatus)
                .currentStatus(OcrPipelineBatch.STATUS_DONE)
                .eventType("COMPLETED")
                .totalFiles(batch != null ? batch.getTotalFiles() : null)
                .message("批次全流程完成")
                .build());
    }
    ...
    private void markBatchFailed(Long batchId, String reason) {
        ...
        publishEvent(OcrPipelineEvent.builder()
                ...
                .appCode(batch != null ? batch.getAppCode() : null)
                .previousStatus(prevStatus)
                .currentStatus(OcrPipelineBatch.STATUS_FAILED)
                .eventType("FAILED")
                .totalFiles(batch != null ? batch.getTotalFiles() : null)
                .message("批次失败: " + truncate(reason, 200))
                .build());
    }

    private void publishEvent(OcrPipelineEvent event) {
        log.debug("[OCR Pipeline] 批次事件: batchNo={}, type={}, previousStatus={}, currentStatus={}, message={}",
                event.getTaskId(), event.getEventType(), event.getPreviousStatus(), event.getCurrentStatus(), event.getMessage());
    }
