package com.sinosig.aip.business.pipeline.scheduler;
 ...
 * </pre>
 *
 * <p>通过配置 {@code aip.pipeline.scheduler.enabled=false} 可关闭定时任务（测试环境可用）。
 */
@Slf4j
@Component
@RequiredArgsConstructor
@ConditionalOnProperty(name = "aip.pipeline.scheduler.enabled", havingValue = "true", matchIfMissing = true)
public class OcrPipelineScheduler {
     ...
     */
    @Scheduled(fixedDelayString = "${aip.pipeline.scheduler.interval-ms:60000}")
    public void compensate() {
        log.debug("[PipelineScheduler] 补偿扫描开始");
        AipPipelineProperties.Scheduler sch = pipelineProperties.getScheduler();
