...
public class ZhengyanTextClassificationTool implements EnterpriseTool {
    ...
    @Value("${aip.tools.zhengyan.text-classification.enabled:false}")
    private boolean enabled;

    @Value("${aip.tools.zhengyan.text-classification.endpoint:}")
    private String endpoint;

    @Value("${aip.tools.zhengyan.text-classification.timeout-ms:15000}")
    private int timeoutMs;

    @Value("${aip.tools.zhengyan.text-classification.auth-header:Authorization}")
    private String authHeader;

    @Value("${aip.tools.zhengyan.text-classification.auth-prefix:Bearer }")
    private String authPrefix;

    @Value("${aip.tools.zhengyan.text-classification.api-key:}")
    private String apiKey;

    @Value("${aip.tools.zhengyan.text-classification.app-id:}")
    private String appId;

    @Value("${aip.tools.zhengyan.text-classification.app-id-header:X-App-Id}")
    ...
    public ToolResponse execute(String params) {
        if (!enabled) {
            return ToolResponse.failure("正言文本分类工具未启用（aip.tools.zhengyan.text-classification.enabled=false）");
        }
        if (endpoint == null || endpoint.isBlank()) {
            return ToolResponse.failure("未配置正言接口地址（aip.tools.zhengyan.text-classification.endpoint）");
        }
