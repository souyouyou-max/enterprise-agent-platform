...
import com.sinosig.aip.engine.rule.util.TextCosineSimilarity;
...
@ConditionalOnProperty(prefix = "aip.audit.collusive.minio", name = "enabled", havingValue = "true", matchIfMissing = false)
public class CollusiveBidMinioRule implements AuditRule {
    ...
    @Value("${aip.audit.collusive.minio.cosine-threshold:0.60}")
    private double cosineThreshold;

    /**
     * 最大两两对比数量，避免 n^2 爆炸；0 表示不限制
     */
    @Value("${aip.audit.collusive.minio.max-pairs:500}")
    ...
    public List<ClueResult> execute(String applyCode) {
        ...
        if (objects.size() < 2) {
            log.info("[{}] 未配置或不足2个MinIO文件，跳过。配置项=aip.audit.collusive.minio.objects", getRuleName());
            return results;
        }
