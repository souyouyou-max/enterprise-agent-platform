...
### ProcurementAuditAgent（`com.enterprise.agent.business.screening`）
...
### ProcurementAuditToolkit（`com.enterprise.agent.business.screening.toolkit`）
...
### AuditEngineService（`com.enterprise.agent.business.screening.service`）
...
- 依赖：`eap-common`、`eap-data-repository`、`eap-data-ingestion`、`eap-engine-rule`、`eap-engine-agent-core`、`eap-engine-llm`
- 被依赖：`eap-business-chat`（`InteractionCenterAgent` 调用 `ProcurementAuditAgent`）
