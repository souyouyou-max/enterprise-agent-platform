 ...
 * 供 eap-scheduler 通过 Feign 调用（避免调度服务直连数据库）。
 */
@Slf4j
...
public class AuditEngineController {
    ...
    // ─── eap-scheduler Feign 调用入口（路径变量风格） ────────────────────────

    /**
     * 供 eap-scheduler 通过 Feign 调用：触发指定申请编码的完整审计
     * 路径：POST /api/v1/audit-engine/trigger/{applyCode}
     */
    @Operation(summary = "触发审计（Feign入口）",
            description = "eap-scheduler 定时任务通过此接口触发审计，applyCode 即申请编码。")
