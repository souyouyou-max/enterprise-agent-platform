...
### CommunicatorAgent（`com.enterprise.agent.business.report`）
...
### CommunicatorToolkit（`com.enterprise.agent.business.report.toolkit`）

支持报告格式化和模板管理。

### RiskManagementController（`com.enterprise.agent.business.report.controller`）

提供报告管理和分发接口（查询/下载/推送）。

## 依赖关系

- 依赖：`eap-common`、`eap-engine-agent-core`、`eap-engine-llm`
- 被依赖：`eap-business-task`（作为流水线末端节点）、`eap-app`
