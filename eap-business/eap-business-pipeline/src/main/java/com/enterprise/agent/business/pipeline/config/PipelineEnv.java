...
/**
 * 流水线相关<strong>环境变量名</strong>（与 eap-app {@code application.yml} 中 {@code ${...}} 占位符一致）。
 * <p>
 * 例如 {@link #PIPELINE_ANALYSIS_MULTIMODAL_PROMPT_KEY} 在 YAML 中写作：
 * {@code eap.pipeline.analysis.multimodal-prompt-key: ${PIPELINE_ANALYSIS_MULTIMODAL_PROMPT_KEY:default}}
 * 启动时由 Spring 解析进 {@link EapPipelineProperties.Analysis#setMultimodalPromptKey(String)}，
 * 业务代码只读 {@link EapPipelineProperties#getAnalysis()}，不直接读取该环境变量名。
 */
public final class PipelineEnv {
    ...
    /**
     * 覆盖 {@code eap.pipeline.analysis.multimodal-prompt-key}（如 {@code default}、{@code bid_screening}）。
     */
    public static final String PIPELINE_ANALYSIS_MULTIMODAL_PROMPT_KEY = "PIPELINE_ANALYSIS_MULTIMODAL_PROMPT_KEY";

    /**
     * 注入 {@code eap.pipeline.analysis.max-total-analysis-pages}（语义分析最多处理多少页分片）。
     */
    public static final String PIPELINE_MAX_TOTAL_ANALYSIS_PAGES = "PIPELINE_MAX_TOTAL_ANALYSIS_PAGES";

    /**
     * 批次 {@code extra_info} 中<strong>可选</strong>覆盖话术模板 key（对应 {@code eap.pipeline.prompts.templates} 的键名）。
     * <p><strong>默认</strong>：不传此字段时，使用配置文件 {@code eap.pipeline.analysis.multimodal-prompt-key}
    ...
    /**
     * 批次 {@code extra_info} 中多套话术模板 key 数组（对应 {@code eap.pipeline.prompts.templates}），
    ...
    /** 是否自动跑 OCR（缺省跟全局 {@code eap.pipeline.ocr.enabled}） */
