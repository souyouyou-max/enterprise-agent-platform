...
- 依赖：`eap-common`（`LlmService`、`AgentRole`、`ResponseResult`）、`eap-engine-agent-core`（`BaseAgent`、`AgentContext`、`AgentResult`）、`spring-ai-starter-model-openai`
- 被依赖：`eap-business-chat`（`AgentOrchestrationToolkit`）、`eap-app`（运行时装配）
