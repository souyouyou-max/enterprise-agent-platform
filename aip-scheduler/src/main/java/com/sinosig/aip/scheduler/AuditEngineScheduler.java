...
import com.sinosig.aip.scheduler.client.AuditEngineClient;
 ...
 * 审计引擎定时调度器
 *
 * <p>通过 Feign 调用 aip-app 的审计触发接口，不再直连数据库。
 * 调用链路：aip-scheduler → (HTTP/Feign) → aip-app → DB
 */
@Slf4j
@Component
...
public class AuditEngineScheduler {
    ...
    @Value("${aip.audit.apply-codes:APP001}")
     ...
     * 定时全量审计任务
     */
    @Scheduled(fixedDelayString = "${aip.audit.interval-seconds:3600}000",
               initialDelayString = "${aip.audit.initial-delay-seconds:30}000")
