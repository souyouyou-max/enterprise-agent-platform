...
public class DazhiOcrTool implements EnterpriseTool {
    ...
    @Value("${eap.tools.dazhi.ocr.enabled:false}")
    private boolean enabled;

    @Value("${eap.tools.dazhi.ocr.general-url:}")
    private String generalOcrUrl;

    @Value("${eap.tools.dazhi.ocr.app-code:G209-GHQ-CLM-JIHEFENGXIANJIQIREN}")
    ...
    public ToolResponse execute(String params) {
        if (!enabled) {
            return ToolResponse.failure("大智部OCR未启用（eap.tools.dazhi.ocr.enabled=false）");
        }
        if (generalOcrUrl == null || generalOcrUrl.isBlank()) {
            return ToolResponse.failure("未配置大智部通用OCR地址（eap.tools.dazhi.ocr.general-url）");
        }
        try {
