...
### 实体类（`com.enterprise.agent.data.entity`）
...
### Mapper 接口（`com.enterprise.agent.data.mapper`）
...
### 数据服务（`com.enterprise.agent.data.service`）
...
### 工具注册表（`com.enterprise.agent.tools`）
...
- 依赖：`eap-common`
- 被依赖：`eap-data-ingestion`、`eap-engine-rule`、`eap-business-*`、`eap-app`
