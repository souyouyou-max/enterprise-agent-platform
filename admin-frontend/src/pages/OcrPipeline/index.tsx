...
/** 与后端合并逻辑一致：空字符串表示不传该键（沿用全局 aip.pipeline） */
  ...
  const fetchProgress = useCallback(async (bn: string, options?: { silent?: boolean }) => {
    ...
    try {
      const { data: body } = await apiFetch<{ code: number; message?: string; data: BatchProgressRaw }>(
        javaBaseUrl, `/api/v1/aip/pipeline/batch/${bn}`,
      )
      if (body.code === 200) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: body } = await apiFetch<{ code: number; message?: string; data: { batchNo: string } }>(
        javaBaseUrl, '/api/v1/aip/pipeline/submit-batch', {
          method: 'POST',
          ...
          <Col xs={24} lg={10}>
            <Form layout="vertical" size="small">
              ...
              <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>
                本批流水线（可选；未选则与后端全局 aip.pipeline 一致）
              </Text>
              <Row gutter={[8, 8]}>
                <Col span={24}>
                  <Space wrap size={[8, 8]}>
                ...
                <Col span={24}>
                  <Form.Item
                    label="话术模板（语义分析）"
                    tooltip="不勾选时使用服务端 aip.pipeline.analysis.multimodal-prompt-key；勾选后本批使用所选模板"
        ...
        <Card
          ...
          extra={
            <Space>
              ...
              {/* 手动触发按钮（仅在卡住时使用） */}
              <Button size="small" onClick={() => trigger(`/api/v1/aip/pipeline/batch/${batchNo}/trigger-ocr`, 'OCR')}>
                触发 OCR
              </Button>
              <Button size="small" onClick={() => trigger(`/api/v1/aip/pipeline/batch/${batchNo}/trigger-analysis`, '分析')}>
                触发分析
              </Button>
              <Button size="small" onClick={() => trigger(`/api/v1/aip/pipeline/batch/${batchNo}/trigger-compare`, '对比')}>
                触发对比
              </Button>
            </Space>
          }
