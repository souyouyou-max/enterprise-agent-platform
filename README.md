...
| 智能体 | 职责 | 核心能力 |
|--------|------|----------|
| **AI交互中心** (`InteractionCenterAgent`) | LLM 驱动的 Agent 编排中心 | 注册四个 Agent 工具，由 LLM 自主决策调用哪些 Agent 及顺序，汇总综合报告 |
...
| **招采稽核** (`ProcurementAuditAgent`) | 招采违规全量检测 | 未招标/化整为零/围标串标/利益输送四类违规识别 |
...
│   └── eap-business-chat/                        # AI交互中心：InteractionCenterAgent（LLM编排）+ AgentOrchestrationToolkit + 多轮对话
│
├── ── 网关层（gateway）─────────────────────────────────────────
...
### 工作原理：LLM 自动 Agent 编排

`InteractionCenterAgent` 是系统的统一对话入口，采用 **LLM 驱动工具调用** 模式：将四个业务 Agent 封装为 `@Tool` 方法，注册给 ChatClient，由 LLM 根据用户意图自主决定调用哪些 Agent 及顺序，无需硬编码路由规则。

```
用户输入（自然语言）
      │
      ▼
InteractionCenterAgent（LLM 理解意图）
      │
      │  LLM 自主决策：需要调用哪些 Agent？
      ▼
┌────────────────────────────────────────────────────────┐
│  discoverClues      analyzeRisk   checkMonitoring  auditProcurement  │
│  线索发现           风险透视        监测预警           招采稽核          │
│  ClueDiscoveryAgent RiskAnalysis  MonitoringAgent  ProcurementAudit  │
└────────────────────────────────────────────────────────┘
      │
      │  AgentOrchestrationToolkit → AgentContext → execute()
      ▼
AI 汇总综合报告（中文）
```
...
| 工具方法 | 对应 Agent | 功能描述 |
|---------|-----------|---------|
| `discoverClues(orgCode)` | `ClueDiscoveryAgent` | 线索发现：扫描采购、财务、合同等数据，发现异常疑点线索 |
| `analyzeRisk(orgCode)` | `RiskAnalysisAgent` | 风险透视：多维风险评分（经营/合规/财务/采购），生成风险报告 |
| `checkMonitoring(orgCode)` | `MonitoringAgent` | 监测预警：检查风险指标阈值，返回预警列表和处置建议 |
...
| 用户意图 | LLM 决策 |
|---------|---------|
| "全面分析 ORG001" | 依次调用全部四个工具，整合综合报告 |
| "ORG001 有没有招采违规？" | 仅调用 `auditProcurement` |
| "ORG002 的风险怎么样？" | 调用 `analyzeRisk`，视情况追加 `checkMonitoring` |
| 未提供机构编码 | LLM 主动追问 orgCode |
...
InteractionCenterAgent.chat()
      ...
      │
      ├─ [3] buildMessages(history)       # 组装历史上下文（最近6条）
      │
      ├─ [4] ChatClient（含工具）调用 LLM
      │       LLM 自主决策 → 调用 AgentOrchestrationToolkit 中的工具
      │       每个工具 → AgentContext.builder() → Agent.execute()
      │
