...
│   └── eap-agent-impl/                  # 四种 Agent + InsightAgent
│       ├── planner/                     # PlannerAgent：目标→子任务分解
│       ├── executor/                    # ExecutorAgent：工具调用+重试
│       ├── reviewer/                    # ReviewerAgent：质量评分
│       ├── communicator/                # CommunicatorAgent：报告生成
│       └── insight/                     # InsightAgent：NL2BI 三步流水线
├── eap-llm/
...
├── eap-knowledge/                       # 企业知识问答（RAG）
│   └── entity/KnowledgeDocument        # 知识文档实体
│   └── repository/KnowledgeRepository  # MyBatis-Plus Mapper
│   └── service/EmbeddingServiceImpl    # Spring AI 向量化
│   └── service/KnowledgeIndexService   # 文档索引+余弦相似度检索
│   └── service/KnowledgeQaService      # RAG 问答编排
├── eap-insight/                         # 智能数据洞察（NL2BI）
│   └── model/InsightRequest,Result     # 请求/响应模型
│   └── service/NlToSqlService          # 自然语言→SQL
│   └── service/DataQueryService        # SQL 安全执行
│   └── service/InsightAnalysisService  # LLM 数据分析报告
├── eap-task/                            # Pipeline编排+Kafka事件
└── eap-app/                             # Spring Boot 主应用（端口8080）
    ├── controller/                      # REST API（含 Knowledge/Insight）
