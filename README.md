...
├── pom.xml                                      # 根 POM（Spring Boot 3.3 + Spring Cloud 2023）
│
├── ── 公共基础层（common）────────────────────────────────────
├── eap-common/                                  # 枚举/异常/统一响应/LlmService接口抽象
│
├── ── 数据层（data）──────────────────────────────────────────
├── eap-data/                                    # 数据实体（entity）+ Mapper + 数据服务 + 企业工具
│   └── src/.../data/
│       ├── entity/                              # PostgreSQL 实体
│       ├── mapper/                              # MyBatis-Plus Mapper
│       ├── service/                             # 数据服务
│       └── tools/                              # 企业工具（SalesData/Employee/CRM/SQL）
│
├── ── 引擎层（engine）─────────────────────────────────────────
├── eap-engine/                                  # 引擎层父模块（pom）
│   ├── eap-engine-llm/                          # LLM：Spring AI 多模型热切换 + Redis缓存
│   ├── eap-engine-agent/                        # Agent 父模块（pom）
│   │   ├── eap-engine-agent-core/               # Agent 核心：BaseAgent/Context/Result/Orchestrator
│   │   └── eap-engine-agent-impl/               # Agent 实现：Planner/Executor/Reviewer/Communicator
│   └── eap-engine-data-service/                 # 数据服务：知识问答（RAG）+ 数据洞察（NL2BI）
│
├── ── 网关层（gateway）────────────────────────────────────────
├── eap-gateway/                                 # API 网关（端口 8080，Spring Cloud Gateway）
│
├── ── 微服务层（service）──────────────────────────────────────
├── eap-service-agent/                           # Agent 任务服务（端口 8081）
├── eap-service-knowledge/                       # 知识问答服务（端口 8082）
├── eap-service-insight/                         # 数据洞察服务（端口 8083）
└── eap-service-chat/                            # AI 交互中心服务（端口 8084）
```
