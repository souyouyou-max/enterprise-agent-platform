...
┌──────────────────────────────────────────────────────────────┐
│  公共库（编译依赖，不独立部署）                                    │
│  eap-common-core  eap-common-ai  eap-llm-service             │
│  eap-enterprise-tools  eap-enterprise-data                   │
│  eap-agent-core  eap-agent-impl  eap-knowledge  eap-insight  │
│  eap-task                                                    │
└──────────────────────────────────────────────────────────────┘
                         │ 打包进各微服务
┌──────────────────────────────────────────────────────────────┐
...
├── ── 公共库（作为依赖，不独立部署）──────────────────────────
├── eap-common/
│   ├── eap-common-core/             # 枚举/异常/统一响应(ResponseResult/PageResult)
│   └── eap-common-ai/               # LlmService/EmbeddingService 接口抽象
├── eap-llm/eap-llm-service/         # Spring AI 实现（OpenAI/Claude/Ollama）+ Redis缓存
├── eap-enterprise/
│   ├── eap-enterprise-tools/        # 企业工具（SalesData/Employee/CRM/SQL）+ Toolkit
│   └── eap-enterprise-data/         # PostgreSQL实体 + MyBatis-Plus Mapper
├── eap-agent/
│   ├── eap-agent-core/              # BaseAgent/AgentContext/AgentResult/Orchestrator/Dispatcher
│   └── eap-agent-impl/              # 五种Agent实现（各含专属Toolkit）
│       ├── planner/                 # PlannerAgent + PlannerToolkit
│       ├── executor/                # ExecutorAgent + ExecutorToolkit
│       ├── reviewer/                # ReviewerAgent + ReviewerToolkit
│       ├── communicator/            # CommunicatorAgent + CommunicatorToolkit
│       └── interaction/             # InteractionCenterAgent + InteractionToolkit
├── eap-knowledge/                   # RAG知识问答（向量化/检索/QA）
├── eap-insight/                     # NL2BI数据洞察（NL2SQL/执行/分析）
├── eap-task/                        # 任务编排 + Kafka事件
│
├── ── 微服务（独立部署）─────────────────────────────────────
├── eap-gateway/                     # API网关（端口8080）
├── eap-service-agent/               # Agent任务服务（端口8081）
├── eap-service-knowledge/           # 知识问答服务（端口8082）
├── eap-service-insight/             # 数据洞察服务（端口8083）
├── eap-service-chat/                # AI交互中心服务（端口8084）
│
